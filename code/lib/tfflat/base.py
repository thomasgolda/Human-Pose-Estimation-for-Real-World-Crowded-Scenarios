...
class Trainer(Base):
    def __init__(self, net, cfg, data_iter=None):
        self.lr_eval = cfg.TRAIN.lr
        self.lr = tf.Variable(cfg.TRAIN.lr, trainable=False)
        self._optimizer = get_optimizer(self.lr, cfg.TRAIN.optimizer)
        self.gpuid = cfg.gpu_ids
    ...
    def _make_data(self):
        ...
        d = Dataset()
        d.setup_paths(self.cfg)
        train_data = d.load_train_data(generate_interference_joints = self.cfg.MODEL.interference_joints, reset_v_flags=self.cfg.TRAIN.reset_v_flags)
        ...
        data_load_thread = BatchData(data_load_thread, self.cfg.TRAIN.batch_size)

        data_load_thread.reset_state()
        dataiter = data_load_thread.get_data()

        return dataiter, math.ceil(len(train_data)/self.cfg.TRAIN.batch_size/self.cfg.num_gpus)
    ...
    def train(self):
        ...
        end_itr = self.itr_per_epoch * self.cfg.TRAIN.end_epoch + 1
        writer = tf.summary.FileWriter(self.cfg.tensorboard_dir, self.sess.graph)
        for itr in range(start_itr, end_itr):
            ...
            setproctitle.setproctitle(self.cfg.EXPERIMENT_NAME)
            ...
            # train one step
            self.gpu_timer.tic()
            _, self.lr_eval, *summary_res, summary = self.sess.run(
                [self.graph_ops[0], self.lr, *self.summary_dict.values(), self.net.merged_summary], feed_dict=feed_dict)
            self.gpu_timer.toc()
            ...
            if itr % self.cfg.tensorboard_update == 0:
                writer.add_summary(summary, itr)
                # for summary in self.net.:
                #     writer.add_summary(summary, itr)
    ...
    def next_feed(self, batch_data=None):
        ...
        if batch_data is None:
            ...
            if self._input_list[0][0].get_shape().as_list()[0] is None:
                ...
                if batch_size > self.cfg.TRAIN.batch_size:
                    self.logger.warning("Current batch_size %d is larger then config batch_size %d." % (batch_size, self.cfg.TRAIN.batch_size))
            else:
                assert self.cfg.TRAIN.batch_size * self.cfg.num_gpus == len(batch_data[0]), \
            ...
            for i in range(self.cfg.num_gpus):
                with tf.device('/gpu:%d' % i):
                    with tf.name_scope('tower_%d' % i) as name_scope:
                        with slim.arg_scope([slim.model_variable, slim.variable], device='/device:CPU:0'):
                            if (self.cfg.MODEL.occluded_detection):
                                self.net.make_occ_network(is_train=False)
                            else:
